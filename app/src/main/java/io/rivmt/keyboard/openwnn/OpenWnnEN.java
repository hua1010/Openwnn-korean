...
import io.rivmt.keyboard.openwnn.EN.DefaultSoftKeyboardEN;
...
public class OpenWnnEN extends OpenWnn {
	...
	private void insertCharToComposingText(char[] chars) {
		...
		if (chars[0] == SPACE[0] || chars[0] == '\u0009') {
			...
			if (mPreviousEventCode == InputJAJPEvent.SELECT_CANDIDATE) {
	...
	@Override synchronized public boolean onEvent(OpenWnnEvent event) {
		...
		case InputJAJPEvent.SELECT_CANDIDATE:
            if (mSymbolMode) {
                ...
                if (mWordSeparators.contains(ev.word.candidate) && 
                    mPreviousEventCode == InputJAJPEvent.SELECT_CANDIDATE) {
                    mInputConnection.deleteSurroundingText(1, 0);
                }
		...
		case InputJAJPEvent.LIST_SYMBOLS:
			commitText(1);
			mComposingText.clear();
